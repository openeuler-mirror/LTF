...
CMDTest_more_GJB() {
    ...
    more_txt="hello world - ${cmd}"
    echo "${more_txt}" >${cmd}testfile

    ${cmd} -V >/dev/null
    [ $? -ne 0 ] && return ${TFAIL}
    ${cmd} -c ${cmd}testfile &>${cmd}testfile1
    cat ${cmd}testfile1 | grep -q "${more_txt}"
    [ $? -ne 0 ] && return ${TFAIL}
    ${cmd} -u ${cmd}testfile &>${cmd}testfile1
    cat ${cmd}testfile1 | grep -q "${more_txt}"
    [ $? -ne 0 ] && return ${TFAIL}
...
CMDMkdirDIR_GJB() {
    if [ ! -d "${CMDTESTDIR_GJB}" ]; then
        mkdir -p ${CMDTESTDIR_GJB}
        if [ $? -ne 0 ]; then
            echo "${CMDTESTDIR_GJB} : Failed to mkdir -p ${CMDTESTDIR_GJB}"
            return ${TCONF}
        fi
    fi
